...
**Note on the “best-scoring submission” behavior.** We observed that the highest-scoring CSV in our experiments often used an *adaptive label count* (sometimes 1 label for very confident items, and occasionally more than 3 labels). This can increase Samples-F1 by trading precision and recall per document: adding more labels can recover missed true labels (recall), while outputting fewer labels on confident examples can keep precision high. However, the course submission rule requires **exactly 2 or 3 labels** per document. Therefore, our final system adopts a **stable top-1 taxonomy** decoding: we anchor on the most confident label (top-1), expand along its taxonomy chain (parent/grandparent), and add at most one extra leaf only when needed. This choice is more stable, taxonomy-consistent by construction, and compliant with the required output format.
